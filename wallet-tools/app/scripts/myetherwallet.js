...
Wallet.prototype.setBalance = function(callback) {
    ...
    ajaxReq.getBalance(parentObj.getAddressString(), function(data) {
        if (data.error) parentObj.balance = data.msg;
            ...
            parentObj.balance = etherUnits.tokCoin(data.data.balance, 'wei');
...
Wallet.prototype.toJSON = function() {
    return {
        ...
        publisher: "Kowala Wallet Tools",
        encrypted: false,
        version: 2
    }
}
Wallet.from = function(input, password) {
...
Wallet.fromKey = function(input, password) {
...
Wallet.walletRequirePass = function(ethjson) {
    ...
    else if (jsonArr.publisher == "Kowala Wallet Tools" && !jsonArr.encrypted) return false;
    else
...
Wallet.getWalletFromPrivKeyFile = function(strjson, password) {
    ...
    else if (jsonArr.hash != null) return Wallet.from(strjson, password);
    else if (jsonArr.publisher == "Kowala Wallet Tools") return Wallet.fromV2(strjson);
    else
        throw globalFuncs.errorMsgs[2];
}
