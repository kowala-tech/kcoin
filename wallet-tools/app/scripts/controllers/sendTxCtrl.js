...
var sendTxCtrl = function($scope, $sce, walletService, $rootScope) {
 ...
 $scope.tokenTx.value = $scope.tx.value;
    ...
    $scope.sendTx = function() {
        ...
        uiFuncs.sendTx($scope.signedTx, function(resp) {
            if (!resp.isError) {
                ...
                var completeMsg = '<p>' + globalFuncs.successMsgs[2] + '</p><p><strong>' + resp.data + '</strong></p><p>' + verifyTxBtn + '</p>';
    ...
    $scope.parseSignedTx = function( signedTx ) {
      ...
      $scope.parsedSignedTx.value         = (txData.value=='0x'||txData.value==''||txData.value==null) ? '0' : etherUnits.tokCoin(new BigNumber(ethFuncs.sanitizeHex(txData.value.toString('hex'))).toString(), 'wei' )
      ...
      $scope.parsedSignedTx.gasPrice.eth  = etherUnits.tokCoin(new BigNumber(ethFuncs.sanitizeHex(txData.gasPrice.toString('hex'))).toString(), 'wei' )
