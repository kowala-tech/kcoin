...
Official implementation of the Kowala protocol.
...
### Full node on the main Kowala network
...
```
$ kusd --testnet --fast --cache=512 console
```
...
As a developer, sooner rather than later you'll want to start interacting with kusd and the Ethereum
network via your own programs and not manually through the console. To aid this, kusd has built in
support for a JSON-RPC based APIs ([standard APIs](https://github.com/ethereum/wiki/wiki/JSON-RPC) and
[kusd specific APIs](https://github.com/ethereum/go-ethereum/wiki/Management-APIs)). These can be
exposed via HTTP, WebSockets and IPC (unix sockets on unix based platforms, and named pipes on Windows).

The IPC interface is enabled by default and exposes all the APIs supported by kusd, whereas the HTTP
...
With the genesis state defined in the above JSON file, you'll need to initialize **every** kusd node
with it prior to starting it up to ensure all blockchain parameters are correctly set:

```
$ kusd init path/to/genesis.json
```
...
it's indeed reachable), start every subsequent kusd node pointed to the bootnode for peer discovery
...
resources (consider running on a single thread, no need for multiple ones either). To start a kusd
instance for mining, run it with all your usual flags, extended by:

```
$ kusd <usual-flags> --mine --minerthreads=1 --etherbase=0x0000000000000000000000000000000000000000
```
