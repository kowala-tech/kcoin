...
#
# The cluster contains a bootnode, a kusd node and multiple swarm nodes, with
...
# static kusd configuration
KUSD_IP="192.168.33.3"
KUSD_RPC_PORT="11223"
KUSD_RPC_URL="http://${KUSD_IP}:${KUSD_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the kusd and
# swarm nodes
start_bootnode() {
...
# start_kusd_node starts a kusd node with --datadir pointing at <base-dir>/kusd
# and a single, unlocked account with password "kusd"
start_kusd_node() {
  local dir="${base_dir}/kusd"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get kusd account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${KUSD_IP}"
    --rpcport   "${KUSD_RPC_PORT}"
    --verbosity "6"
  )

  start_node "kusd" "${KUSD_IP}" "$(which kusd)" ${args[@]}
}
...
create_account() {
  ...
  kusd --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
