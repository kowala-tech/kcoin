...
go_generate: moq go-bindata stringer gencodec mockery ensure_notifications ensure_wallet_backend protoc-gen-go
	egrep -ir --exclude-dir=vendor --exclude-dir=_workspace --exclude-dir=./client/contracts/bindings  --exclude-dir=./client/dashboard --include \*.go "//go:generate" . | sed -E 's/^([^\:]*).*$$/\1/' | uniq | xargs -n 1 go generate
