...
func testDAOForkBlockNewChain(t *testing.T, test int, genesis string, expectBlock *big.Int, expectVote bool) {
	...
	// Start a kusd instance with the requested flags set and immediately terminate
	if genesis != "" {
		...
		kusd := runKusd(t, append(args, []string{"--exec", "2+2", "console"}...)...)
		kusd.WaitExit()
	}
	// Retrieve the DAO config flag from the database
	path := filepath.Join(datadir, "kusd", "chaindata")
	db, err := kusddb.NewLDBDatabase(path, 0, 0)
	if err != nil {
