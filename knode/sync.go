...
func (pm *ProtocolManager) synchronise(peer *peer) {
	...
	pHead, pBlockNumber := peer.Head()

	if pBlockNumber.Cmp(blockNumber) <= 0 {
		return
	}
	...
	if mode == downloader.FastSync {
		// Make sure the peer's fast block we are synchronizing is higher.
		if pm.blockchain.CurrentFastBlock().Number().Cmp(pBlockNumber) >= 0 {
			return 
		}	
	}

	// Run the sync cycle, and disable fast sync if we've went past the pivot block
	if err := pm.downloader.Synchronise(peer.id, pHead, pBlockNumber, mode); err != nil {
		return
	}
	if atomic.LoadUint32(&pm.fastSync) == 1 {
