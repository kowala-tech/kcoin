...
func (kcoin *Kowala) startBloomHandlers() {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-kcoin.shutdownChan:
					return

				case request := <-kcoin.bloomRequests:
					task := <-request
...
func NewBloomIndexer(db kcoindb.Database, size uint64) *core.ChainIndexer {
