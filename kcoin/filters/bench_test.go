...
func benchmarkBloomBits(b *testing.B, sectionSize uint64) {
	...
	db, err := kcoindb.NewLDBDatabase(benchDataDir, 128, 1024)
	if err != nil {
	...
	for i := 0; i < benchFilterCnt; i++ {
		if i%20 == 0 {
			db.Close()
			db, _ = kcoindb.NewLDBDatabase(benchDataDir, 128, 1024)
...
func forEachKey(db kcoindb.Database, startPrefix, endPrefix []byte, fn func(key []byte)) {
...
func clearBloomBits(db kcoindb.Database) {
