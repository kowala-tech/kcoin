...
func New(config *Config, server *kusd.Kowala) (*Service, error) {
	url := config.URL
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	network := fmt.Sprintf("%d", info.(*kusd.NodeInfo).Network)
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	return &blockStats{
		...
		Validator:  "Ricardo Geraldes" //author,
...
// validation layer and reports it to the stats server.
func (s *Service) reportStats(conn *websocket.Conn) error {
	// Gather the syncing and validation infos from the local validator instance
	var (
		validating   bool
		syncing  bool
		gasprice int
	...
	validating = s.kusd.Validator().IsValidating()
