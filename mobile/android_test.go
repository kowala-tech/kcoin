...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Kusd.LightScryptN, Kusd.LightScryptP);
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "kusd-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/ethereum/kusdtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Kusd and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.kowala", "github.com/kowala-tech/kUSD/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "kusd.aar"), "kusd.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "ethereum", "kusdtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
