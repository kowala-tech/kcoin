...
Node.prototype.init = function(data) {
  ...
  if (!_.isUndefined(data.latency)) this.stats.latency = data.latency;
...
Node.prototype.setInfo = function(data, callback) {
  if (!_.isUndefined(data.info)) {
    ...
    if (!_.isUndefined(data.info.canUpdateHistory)) {
      this.info.canUpdateHistory = _.result(
        data,
        "info.canUpdateHistory",
        false
      );
    }
  }
...
Node.prototype.getInfo = function(callback) {
  return {
    ...
    stats: {
      ...
      validating: this.stats.validating,
      syncing: this.stats.syncing,
      peers: this.stats.peers,
      gasPrice: this.stats.gasPrice,
      block: this.stats.block,
      contracts: this.stats.contracts,
...
Node.prototype.setStats = function(stats, history, callback) {
  if (!_.isUndefined(stats)) {
    ...
    this.setContracts(
      _.result(stats, "contracts", this.stats.contracts),
...
Node.prototype.setBlock = function(block, history, callback) {
  if (!_.isUndefined(block) && !_.isUndefined(block.number)) {
    if (
      !_.isEqual(history, this.history) ||
...
Node.prototype.setContracts = function(contracts, callback) {
  if (!_.isUndefined(contracts)) {
    if ((!_.isEqual(contracts), this.stats.contracts)) {
      this.stats.contracts = contracts;
      callback(null, this.getContractsStats());
    } else {
      callback(null, null);
    }
  } else {
    callback("Contracts undefined", null);
  }
};
...
Node.prototype.setHistory = function(history) {
  ...
  this.stats.propagationAvg =
    positives.length > 0 ? Math.round(_.sum(positives) / positives.length) : 0;
  positives = null;
...
Node.prototype.setBasicStats = function(stats, callback) {
  if (!_.isUndefined(stats)) {
    if (
      !_.isEqual(stats, {
        active: this.stats.active,
        validation: this.stats.validating,
        peers: this.stats.peers,
        gasPrice: this.stats.gasPrice,
        uptime: this.stats.uptime
      })
      ...
      this.stats.validating = stats.validating;
      this.stats.syncing = !_.isUndefined(stats.syncing)
        ? stats.syncing
        : false;
...
Node.prototype.getContractsStats = function() {
  return {
    id: this.id,
    contracts: this.stats.contracts
  };
...
Node.prototype.calculateUptime = function() {
  ...
  return Math.round(
    (this.uptime.up / (this.uptime.lastUpdate - this.uptime.started)) * 100
  );
};
...
Node.prototype.canUpdate = function() {
  ...
  return (
    ...
    (this.stats.syncing === false && this.stats.peers > 0) ||
    false
  );
};
