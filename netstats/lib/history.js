...
History.prototype.add = function(block, id, trusted, addingHistory) {
  ...
  if (
    ...
    if (historyBlock) {
        ...
        if (forkIndex >= 0 && !_.isUndefined(historyBlock.forks[forkIndex])) {
          ...
          if (prevBlock) {
            ...
            if (block.number < this.bestBlock().height)
              block.time = Math.max(
                (block.timestamp - prevBlock.block.timestamp) * 1000,
                0
              );
          } else {
            block.time = 0;
      ...
      if (
        trusted &&
        !compareBlocks(historyBlock.block, historyBlock.forks[forkIndex])
      ) {
...
function compareForks(historyBlock, block2) {
  ...
  for (var x = 0; x < historyBlock.forks.length; x++)
    if (compareBlocks(historyBlock.forks[x], block2)) return x;
...
History.prototype.bestBlockNumber = function() {
  ...
  if (!_.isUndefined(best.height)) return best.height;
...
History.prototype.worstBlockNumber = function(trusted) {
  ...
  if (!_.isUndefined(worst.height)) return worst.height;
...
History.prototype.getNodePropagation = function(id) {
  ...
  var sorted = _(this._items)
    ...
    .forEach(function(item, key) {
      ...
      if (index >= 0) {
        var tmpPropagation = _.result(
          _.find(item.propagTimes, "node", id),
          "propagation",
          false
        );
...
History.prototype.getValidatorsCount = function() {
  var validators = _(this._items)
    ...
    .slice(0, MAX_BINS)
    .map(function(item) {
      return item.block.validator;
    })
    .value();

  var validatorsCount = [];

  _.forEach(_.countBy(validators), function(cnt, validator) {
    validatorsCount.push({ validator: validator, name: false, blocks: cnt });
  });

  return _(validatorsCount)
