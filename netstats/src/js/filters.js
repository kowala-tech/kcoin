...
angular
  ...
  .filter("validatingClass", function() {
    return function(validating, active) {
  ...
  .filter("nodeVersion", function($sce) {
    return function(version) {
      if (version) {
        ...
        if (tmp[2] && tmp[2].indexOf("Linux") === 0) tmp[2] = "linux";

        if (tmp[2] && tmp[2].indexOf("Darwin") === 0) tmp[2] = "darwin";
  ...
  .filter("gasPriceFilter", [
    ...
    function(filter) {
      ...
      return function(price) {
        ...
        if (price.length < 7) return numberFilter(price / 1000) + " kwei";

        if (price.length < 10) return numberFilter(price / 1000000) + " mwei";
  ...
  .filter("latencyFilter", function() {
    return function(stats) {
      if (stats.active === false) return "offline";
      else return stats.latency + " ms";
    };
  })
  .filter("latencyClass", function() {
    return function(stats) {
      if (stats.active === false) return "text-danger";
  ...
  .filter("geoTooltip", function() {
    return function(node) {
      ...
      if (node.info.protocol !== "") {
        string =
          "Protocol: <b>" +
          (typeof node.info.protocol !== "undefined"
            ? node.info.protocol
            : "-") +
          "</b>";
  ...
  .filter("validatorNameFilter", function() {
