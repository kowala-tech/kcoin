// Package kusdclient provides a client for the Kowala RPC API.
package kusdclient
	...
	if len(body.UncleHashes) > 0 {
		...
		for i := range reqs {
			reqs[i] = rpc.BatchElem{
				Method: "kusd_getUncleByBlockHashAndIndex",
...
func (ec *Client) TransactionByHash(ctx context.Context, hash common.Hash) (tx *types.Transaction, isPending bool, err error) {
	var raw json.RawMessage
	err = ec.c.CallContext(ctx, &raw, "kusd_getTransactionByHash", hash)
	if err != nil {
...
func (ec *Client) TransactionInBlock(ctx context.Context, blockHash common.Hash, index uint) (*types.Transaction, error) {
	...
	err := ec.c.CallContext(ctx, &tx, "kusd_getTransactionByBlockHashAndIndex", blockHash, hexutil.Uint64(index))
	if err == nil {
		if tx == nil {
			return nil, kowala.NotFound
		} else if _, r, _ := tx.RawSignatureValues(); r == nil {
