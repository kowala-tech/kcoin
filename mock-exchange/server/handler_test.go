...
func TestFetchDataHandler_ItNeedsToBePostMethodRequest(t *testing.T) {
...
func TestFetchDataHandler_WeFetchDataWeWantToDisplayAndIsSavedToCache(t *testing.T) {
...
func TestGetRatesHandler_ItReturnsTheCorrectCodifiedResponse(t *testing.T) {
	c := cache.New(5*time.Minute, 10*time.Minute)
	transformer := exrates.Transformer{}

	cachedRequest := app.Request{
		Sell: []app.RateValue{
			{
				Amount: 0.358,
				Rate:   6326.83689418,
	...
	c.Set(app.CacheRequestKey, cachedRequest, cache.NoExpiration)
	...
	assert.Equal(t, expectedTransformedResponse, rr.Body.String())
}
