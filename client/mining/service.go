...
func New(ctx *node.ServiceContext, config *Config) (*Service, error) {
	...
	binding, err := consensus.Bind(NewContractBackend(service.kowalaService.APIBackend()), service.kowalaService.BlockChain().Config().ChainID)
	if err != nil {
		return nil, err
	}
	service.validatorMgr = binding.(*consensus.Consensus)

	service.validator = validator.New(service.kowalaService, service.validatorMgr, service.globalEvents, service.log)
	...
	service.serverPool = newServerPool(service.kowalaService.ChainDb(), service.shutdownChan, new(sync.WaitGroup))
...
func (s *Service) Start(srvr *p2p.Server) error {
	...
	if srvr.DiscoveryV5 {
		topic := discv5.DiscoveryTopic(s.kowalaService.BlockChain().Genesis().Hash(), ProtocolName, Validator1)

		go func() {
			srvr.DiscV5.RegisterTopic(topic, s.shutdownChan)
		}()

		s.serverPool.start(srvr, topic)
	}
...
func (s *Service) Protocols() []p2p.Protocol {
	return s.protocolMgr.SubProtocols
}
