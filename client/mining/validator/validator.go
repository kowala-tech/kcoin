...
func New(backend Backend, consensus *consensus.Consensus, globalEvents *event.TypeMux, log log.Logger) *validator {
	validator := &validator{
		backend:      backend,
...
func (val *validator) sync() {
	if err := SyncWaiter(val.globalEvents); err != nil {
		val.log.Warn("Failed to sync with network", "err", err)
	} else {
		val.finishedSync()
	}
}
...
func (val *validator) Start(walletAccount accounts.WalletAccount, deposit *big.Int) {
	if val.Validating() {
		val.log.Warn("failed to start the validator - the state machine is already running")
		return
	}
	...
	if atomic.LoadInt32(&val.canStart) == 0 {
		val.log.Info("network syncing, will start validator afterwards")
		return
	}
...
func (val *validator) run() {
	...
	val.log.Info("Starting the consensus state machine")
...
func (val *validator) Stop() error {
	...
	val.log.Info("Stopping consensus validator")

	val.leave()
	val.wg.Wait() // waits until the validator is no longer registered as a voter.

	atomic.StoreInt32(&val.shouldStart, 0)
	val.log.Info("Consensus validator stopped")
	return nil
}
...
func (val *validator) Pending() (*types.Block, *state.StateDB) {
	...
	if err != nil {
		val.log.Crit("Failed to fetch the latest state", "err", err)
	}
...
func (val *validator) restoreLastCommit() {
	...
	if err != nil {
		val.log.Crit("Failed to access the voters checksum", "err", err)
	}

	if err := val.updateValidators(checksum, true); err != nil {
		val.log.Crit("Failed to update the validator set", "err", err)
	}
...
func (val *validator) init() error {
	...
	if err != nil {
		val.log.Error("Failed to create voting system", "err", err)
		return nil
	}
	...
	if err = val.makeCurrent(parent); err != nil {
		val.log.Error("Failed to create mining context", "err", err)
		return nil
	}
...
func (val *validator) AddProposal(proposal *types.Proposal) error {
	...
	val.log.Info("Received Proposal")
...
func (val *validator) AddVote(vote *types.Vote) error {
	...
	if err := val.votingSystem.Add(addressVote); err != nil {
		val.log.Error("cannot add the vote", "err", err)
	}
...
func (val *validator) commitTransactions(mux *event.TypeMux, txs *types.TransactionsByPriceAndNonce, bc *core.BlockChain, coinbase common.Address) {
	...
	for {
		...
		case core.ErrGasLimitReached:
			...
			val.log.Trace("Gas limit exceeded for current block", "sender", from)
			txs.Pop()

		case core.ErrNonceTooLow:
			// New head notification data race between the transaction pool and miner, shift
			val.log.Trace("Skipping transaction with low nonce", "sender", from, "nonce", tx.Nonce())
			txs.Shift()

		case core.ErrNonceTooHigh:
			// Reorg notification data race between the transaction pool and miner, skip account =
			val.log.Trace("Skipping account with hight nonce", "sender", from, "nonce", tx.Nonce())
			txs.Pop()
		...
		default:
			...
			val.log.Debug("Transaction failed, account skipped", "hash", tx.Hash(), "err", err)
			txs.Shift()
		}
	}
...
func (val *validator) leave() {
	...
	if err != nil {
		val.log.Error("failed to leave the election", "err", err)
	}
	receipt, err := tx.WaitMinedWithTimeout(val.backend, txHash, txConfirmationTimeout)
	if err != nil {
		val.log.Error("Failed to verify the voter deregistration", "err", err)
	}
	if receipt.Status == types.ReceiptStatusFailed {
		val.log.Error("Failed to deregister validator - receipt status failed")
	}
}

func (val *validator) createProposalBlock() *types.Block {
	if val.lockedBlock != nil {
		val.log.Info("Picking a locked block")
...
func (val *validator) createBlock() *types.Block {
	val.log.Info("Creating a new block")
	// new block header
	parent := val.chain.CurrentBlock()
	...
	if err := val.engine.Prepare(val.chain, header); err != nil {
		val.log.Error("Failed to prepare header for mining", "err", err)
		return nil
	}

	pending, err := val.backend.TxPool().Pending()
	if err != nil {
		val.log.Crit("Failed to fetch pending transactions", "err", err)
	}

	txs := types.NewTransactionsByPriceAndNonce(val.signer, pending)
	val.commitTransactions(val.globalEvents, txs, val.chain, val.walletAccount.Account().Address)

	// Create the new block to seal with the consensus engine
	var block *types.Block
	if block, err = val.engine.Finalize(val.chain, header, val.state, val.txs, commit, val.receipts); err != nil {
		val.log.Crit("Failed to finalize block for sealing", "err", err)
	}
...
func (val *validator) propose() {
	...
	if err != nil {
		val.log.Crit("Failed to get the block as a set of fragments of information", "err", err)
	}
	...
	if err != nil {
		val.log.Crit("Failed to sign the proposal", "err", err)
	}
...
func (val *validator) preVote() {
	...
	case val.lockedBlock != nil:
		val.log.Debug("Locked Block is not nil, voting for the locked block")
		vote = val.lockedBlock.Hash()
	case val.block == nil:
		val.log.Debug("Proposal's block is nil, voting nil")
		vote = common.Hash{}
	default:
		val.log.Debug("Voting for the proposal's block")
		vote = val.block.Hash()
	}
...
func (val *validator) preCommit() {
	...
	if err != nil {
		val.log.Crit("Error while preCommit stage", "err", err)
	}
	...
	case currentLeader == common.Hash{}:
		val.log.Debug("Majority of validators pre-voted nil")
		// unlock locked block
		if val.lockedBlock != nil {
			val.lockedRound = 0
			val.lockedBlock = nil
	...
	case val.lockedBlock != nil && currentLeader == val.lockedBlock.Hash():
		val.log.Debug("Majority of validators pre-voted the locked block", "block", val.lockedBlock.Hash())
	...
	case val.block != nil && currentLeader == val.block.Hash():
		val.log.Debug("Majority of validators pre-voted the proposed block", "block", val.block.Hash())
		// lock block
		val.lockedRound = val.round
	...
	default:
		...
		// unlock locked block
		val.log.Debug("preCommit default case")
		val.lockedRound = 0
		val.lockedBlock = nil
		val.block = nil
	}
...
func (val *validator) vote(vote *types.Vote) {
	...
	if err != nil {
		val.log.Crit("Failed to sign the vote", "err", err)
	}

	addressVote, err := types.NewAddressVote(val.signer, signedVote)
	if err != nil {
		val.log.Crit("Failed to make address Vote", "err", err)
	}

	err = val.votingSystem.Add(addressVote)
	if err != nil {
		val.log.Error("Failed to add own vote to voting table",
...
func (val *validator) AddBlockFragment(blockNumber *big.Int, round uint64, fragment *types.BlockFragment) error {
	...
	if val.blockFragments.HasAll() {
		...
		if err != nil {
			...
			val.log.Error("error while adding a new block fragment", "err", err, "round", round, "block", blockNumber, "fragment", fragment)
			return err
		}
		...
		if err == nil {
			...
			if err != nil {
				...
				val.log.Error("error while validating a block body",
		...
		if err != nil {
			val.log.Error("Failed to process the block", "err", err,
				"round", round, "block", blockNumber, "fragment", fragment, "block", block)

			val.log.Crit("Failed to process the block", "err", err)
		}
		...
		if err != nil {
			...
			val.log.Error("Failed to validate the state", "err", err,
				"round", round, "block", blockNumber, "fragment", fragment, "block", block)

			val.log.Crit("Failed to validate the state", "err", err)
		}
...
func (val *validator) updateValidators(checksum [32]byte, genesis bool) error {
	...
	if val.voters != nil {
		val.log.Debug("voting. updating a list of validators", "was", val.voters.Len(), "now", validators.Len())
	} else {
		val.log.Debug("voting. updating a list of validators", "was", "nil", "now", validators.Len())
	}
