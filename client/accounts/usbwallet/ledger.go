...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and Kowala address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an Kowala transaction after having the user validate the parameters
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Kowala app in browser mode", w.failure
	}
	if w.offline() {
		return "Kowala app offline", w.failure
	}
	return fmt.Sprintf("Kowala app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whether the wallet and the Kowala app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// Kowala app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the Kowala app's version, will fail prior to v1.0.2
...
// and returning the Kowala address located on that derivation path.
...
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the Kowala app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the Kowala wallet app running
...
// ledgerDerive retrieves the currently active Kowala address from a Ledger
...
//   Kowala address length 	 | 1 byte
//   Kowala address          | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the Kowala hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an Kowala address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	if txrlp, err = rlp.EncodeToBytes([]interface{}{tx.Nonce(), tx.ComputeLimit(), tx.To(), tx.Value(), tx.Data(), chainID, big.NewInt(0), big.NewInt(0)}); err != nil {
