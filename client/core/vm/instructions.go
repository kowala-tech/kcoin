...
func opCreate(pc *uint64, vm *VM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	var (
		...
		resources    = contract.ComputationalResources
	)

	resources -= resources / 64

	contract.UseResources(resources)
	res, addr, returnResources, suberr := vm.Create(contract, input, resources, value)
	// Push item on the stack based on the returned error. If the ruleset is
	// homestead we must check for CodeStoreOutOfComputationalResourcesError (homestead only
	// rule) and treat as an error, if the ruleset is frontier we must
	// ignore this error and pretend the operation was successful.
	if suberr == ErrCodeStoreOutOfComputationalResources {
		stack.push(vm.interpreter.intPool.getZero())
	} else if suberr != nil && suberr != ErrCodeStoreOutOfComputationalResources {
		stack.push(vm.interpreter.intPool.getZero())
	} else {
		stack.push(addr.Big())
	}
	contract.ComputationalResources += returnResources
