...
// requires a deterministic computational resource count based on the input size of the Run method of the
// contract.
...
func RunPrecompiledContract(p PrecompiledContract, input []byte, contract *Contract) (ret []byte, err error) {
	effort := p.RequiredComputationalEffort(input)
	if contract.UseResource(effort) {
		return p.Run(input)
	}
