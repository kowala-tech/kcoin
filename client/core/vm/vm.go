...
// revert-state-and-consume-all-computational-resource operation, no checks on
...
type VM struct {
	...
	abort int32
	// callResourceTemp holds the computational resource available for the current call. This is needed because the
	// available resource is calculated in resourceCall* according to the 63/64 rule and later
	// applied in opCall*.
...
func (vm *VM) Call(caller ContractRef, addr common.Address, input []byte, computeLimit uint64, value *big.Int) (ret []byte, leftOverResource uint64, err error) {
	...
	if vm.vmConfig.Debug && vm.depth == 0 {
		...
		defer func() { // Lazy evaluation of the parameters
			vm.vmConfig.Tracer.CaptureEnd(ret, computeLimit-contract.ComputationalResource, time.Since(start), err)
		}()
	}
	ret, err = run(vm, contract, input)

	// When an error was returned by the VM or when setting the creation code
	// above we revert to the snapshot and consume any resource remaining. Additionally
	// when we're in homestead this also counts for code storage resource errors.
	if err != nil {
		vm.StateDB.RevertToSnapshot(snapshot)
		if err != errExecutionReverted {
			contract.UseResource(contract.ComputationalResource)
		}
	}
	return ret, contract.ComputationalResource, err
}
...
func (vm *VM) CallCode(caller ContractRef, addr common.Address, input []byte, computeLimit uint64, value *big.Int) (ret []byte, leftOverResource uint64, err error) {
	if vm.vmConfig.NoRecursion && vm.depth > 0 {
		return nil, computeLimit, nil
	}
...
func (vm *VM) DelegateCall(caller ContractRef, addr common.Address, input []byte, computeLimit uint64) (ret []byte, leftOverResource uint64, err error) {
	if vm.vmConfig.NoRecursion && vm.depth > 0 {
		return nil, computeLimit, nil
	}
	// Fail if we're trying to execute above the call depth limit
...
func (vm *VM) StaticCall(caller ContractRef, addr common.Address, input []byte, computeLimit uint64) (ret []byte, leftOverResource uint64, err error) {
	if vm.vmConfig.NoRecursion && vm.depth > 0 {
		return nil, computeLimit, nil
	}
	// Fail if we're trying to execute above the call depth limit
...
func (vm *VM) Create(caller ContractRef, code []byte, computeLimit uint64, value *big.Int) (ret []byte, contractAddr common.Address, leftOverResource uint64, err error) {
	...
	// be stored due to not enough resource set an error and let it be handled
	// by the error checking condition below.
	if err == nil && !maxCodeSizeExceeded {
		createDataEffort := uint64(len(ret)) * effort.CreateData
		if contract.UseResource(createDataEffort) {
			vm.StateDB.SetCode(contractAddr, ret)
		} else {
			err = ErrCodeStoreOutOfComputationalResource
		}
	}
	...
	// above we revert to the snapshot and consume any resource remaining. Additionally
	// when we're in homestead this also counts for code storage resource errors.
	if maxCodeSizeExceeded || (err != nil && err != ErrCodeStoreOutOfComputationalResource) {
		vm.StateDB.RevertToSnapshot(snapshot)
		if err != errExecutionReverted {
			contract.UseResource(contract.ComputationalResource)
		}
	...
	return ret, contractAddr, contract.ComputationalResource, err
}
