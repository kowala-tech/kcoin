...
type (
	...
	effortFunc          func(params.ComputationalEffortTable, *VM, *Contract, *Stack, *Memory, uint64) (uint64, error) // last parameter is the requested memory size as a uint64
...
type operation struct {
	...
	// computational effort is the computational effort function and returns the required computational effort for execution
...
func NewAndromedaInstructionSet() [256]operation {
	return [256]operation{
		STOP: {
			execute:             opStop,
			computationalEffort: constCompEffortFunc(0),
		...
		ADD: {
			execute:             opAdd,
			computationalEffort: constCompEffortFunc(CompEffortFastestStep),
			validateStack:       makeStackFunc(2, 1),
			valid:               true,
		},
		MUL: {
			execute:             opMul,
			computationalEffort: constCompEffortFunc(CompEffortFastStep),
			validateStack:       makeStackFunc(2, 1),
			valid:               true,
		},
		SUB: {
		...
		ADDMOD: {
			execute:             opAddmod,
			computationalEffort: constCompEffortFunc(CompEffortMidStep),
		...
		ADDRESS: {
			execute:             opAddress,
			computationalEffort: constCompEffortFunc(CompEffortQuickStep),
		...
		BLOCKHASH: {
			execute:             opBlockhash,
			computationalEffort: constCompEffortFunc(CompEffortExtStep),
			validateStack:       makeStackFunc(1, 1),
			valid:               true,
		},
		COINBASE: {
		...
		JUMPI: {
			execute:             opJumpi,
			computationalEffort: constCompEffortFunc(CompEffortSlowStep),
			validateStack:       makeStackFunc(2, 0),
			jumps:               true,
			valid:               true,
		},
		...
		JUMPDEST: {
			execute:             opJumpdest,
			computationalEffort: constCompEffortFunc(params.JumpdestCompEffort),
			validateStack:       makeStackFunc(0, 0),
			valid:               true,
		},
		PUSH1: {
