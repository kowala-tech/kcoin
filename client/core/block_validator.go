...
// transition, such as the computational resource usage, the receipt roots and the state root
...
func (v *BlockValidator) ValidateState(block, parent *types.Block, statedb *state.StateDB, receipts types.Receipts, resourceUsage uint64) error {
	...
	if block.ResourceUsage() != resourceUsage {
		return fmt.Errorf("invalid computational resources used (remote: %d local: %d)", block.ResourceUsage(), resourceUsage)
	}
