...
func (pool *TxPool) enqueueTx(hash common.Hash, tx *types.Transaction) (bool, error) {
	...
	inserted, old := pool.pending[from].Add(tx)
	if !inserted {
...
// Stats retrieves the current pool stats, namely the number of executable and the
...
func (pool *TxPool) stats() (int, int) {
	executable := 0
	for _, list := range pool.executable {
		executable += list.Len()
	}
	pending := 0
...
var (
	...
	// ErrIntrinsicCompEffort is returned if the transaction is specified to use less resources
	// than required to start the invocation.
	ErrIntrinsicCompEffort = errors.New("compute limit too low")

	// ErrExceedsComputeCapacity is returned if a transaction's required computational
	// effort exceeds the compute capacity per block.
	ErrExceedsComputeCapacity = errors.New("exceeds compute capacity")
...
type TxPool struct {
	...
	pending // all transactions awaiting acceptance
...
// pending/ready queue and any subsequent transactions that become unexecutable
// are moved back into the future queue.
func (pool *TxPool) demoteUnexecutables() {
	// Iterate over all accounts and remove any non-executable transactions
	for addr, list := range pool.ready {
		nonce := pool.currentState.GetNonce(addr)
...
// promoteTx adds a transaction to the executable (ready) list of transactions
// and returns whether it was inserted or an older was better.
//
// Note, this method assumes the pool lock is held!
func (pool *TxPool) promoteTx(addr common.Address, hash common.Hash, tx *types.Transaction) bool {
	// Try to insert the transaction into the ready queue
