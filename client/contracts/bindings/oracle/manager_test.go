...
func (suite *OracleMgrSuite) BeforeTest(suiteName, testName string) {
	...
	_, _, oracleMgrContract, err := oracle.DeployOracleMgr(transactOpts, suite.backend, big.NewInt(int64(maxNumOracles)), mockAddr)
	req.NoError(err)
...
func (suite *OracleMgrSuite) TestDeploy() {
	...
	_, _, oracleMgrContract, err := oracle.DeployOracleMgr(transactOpts, suite.backend, maxNumOracles, mockAddr)
	req.NoError(err)
	req.NotNil(oracleMgrContract)
...
func (suite *OracleMgrSuite) TestDeploy_MaxNumOraclesEqualZero() {
	...
	_, _, _, err = oracle.DeployOracleMgr(transactOpts, suite.backend, maxNumOracles, mockAddr)
