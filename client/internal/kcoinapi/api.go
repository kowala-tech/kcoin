...
func (s *PublicKowalaAPI) Syncing() (interface{}, error) {
...
//   keccak256("\x19Kowala Signed Message:\n"${message length}${message}).
//
...
// Sign calculates an Kowala ECDSA signature for:
// keccack256("\x19Kowala Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/kowala/kcoin/wiki/Management-APIs#personal_sign
...
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	...
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Kowala signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Kowala blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Kowala blockchain API.
...
func (s *PublicBlockChainAPI) doCall(ctx context.Context, args CallArgs, blockNr rpc.BlockNumber, vmCfg vm.Config, timeout time.Duration) ([]byte, uint64, bool, error) {
	...
	// Set default compute limit if not set
	...
	// or, in case of unmetered computational resource, setup a context with a timeout.
	...
	return res, resourceUsage, failed, err
}
...
// EstimateComputationalEffort returns an estimate of the computational effort required to execute the
// given transaction against the current pending block.
func (s *PublicBlockChainAPI) EstimateComputationalEffort(ctx context.Context, args CallArgs) (hexutil.Uint64, error) {
	// Binary search the resource requirement, as it may be higher than the amount used
	var (
		lo  uint64 = effort.Tx - 1
		hi  uint64
		cap uint64
	)
	if uint64(args.ComputeLimit) >= effort.Tx {
	...
	// Create a helper to check if a resource allowance results in an executable transaction
...
func (api *PublicDebugAPI) SeedHash(ctx context.Context, number uint64) (string, error) {
	...
	return "", nil //fmt.Sprintf("0x%x", ethash.SeedHash(number)), nil
}
...
// Version returns the current kowala protocol version.
