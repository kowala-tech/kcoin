...
var (
	...
	srcMapFlag = flag.String("srcmap", "", "Path to the Kowala contract source map used to debug.")
...
func main() {
	...
	if *solFlag != "" || *abiFlag == "-" {
		...
		srcMap := []byte{}
		if *srcMapFlag != "" {
			if srcMap, err = ioutil.ReadFile(*srcMapFlag); err != nil {
				fmt.Printf("Failed to read input bytecode: %v\n", err)
				os.Exit(-1)
			}
		}
		srcMaps = append(srcMaps, string(srcMap))
