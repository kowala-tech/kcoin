...
func testDAOForkBlockNewChain(t *testing.T, test int, genesis string, expectBlock *big.Int, expectVote bool) {
	...
	// Start a kcoin instance with the requested flags set and immediately terminate
	if genesis != "" {
		...
		kcoin := runKusd(t, append(args, []string{"--exec", "2+2", "console"}...)...)
		kcoin.WaitExit()
	}
	// Retrieve the DAO config flag from the database
	path := filepath.Join(datadir, "kcoin", "chaindata")
	db, err := kcoindb.NewLDBDatabase(path, 0, 0)
	if err != nil {
