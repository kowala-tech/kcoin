...
type CallMsg struct {
	...
	ComputeLimit uint64          // if 0, the call executes with near-infinite gas
...
// ComputationalEffortEstimator wraps EstimateComputationalEffort, which tries to estimate the required
// computational effort to execute a specific transaction based on the pending state. There is no
// guarantee that this is the true gas limit requirement as other transactions may be added or
// removed by validators, but it should provide a basis for setting a reasonable default.
type ComputationalEffortEstimator interface {
