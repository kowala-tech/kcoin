...
func (val *validator) commitTransactions(mux *event.TypeMux, txs *core.TransactionsByTimestampAndNonce, bc *core.BlockChain, coinbase common.Address) {
	crpool := new(core.ComputationalResourcePool).AddResource(params.ComputeCapacity)
	...
	for {
		...
		logs, err := val.commitTransaction(tx, bc, coinbase, crpool)
		switch err {
		case core.ErrComputeCapacityReached:
			// Pop the current out-of-computational-resource transaction without shifting in the next from the account
...
func (val *validator) commitTransaction(tx *types.Transaction, bc *core.BlockChain, coinbase common.Address, crpool *core.ComputationalResourcePool) ([]*types.Log, error) {
