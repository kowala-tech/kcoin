...
type Tracer struct {
	initialized bool // Flag whether the context was already initialized from the VM
	...
	resourceValue *uint   // Swappable resource value wrapped by a log accessor
...
func New(code string) (*Tracer, error) {
	...
	tracer.vm.PushGlobalGoFunction("isPrecompiled", func(ctx *duktape.Context) int {
		_, ok := vm.PrecompiledContractsAndromeda[common.BytesToAddress(popSlice(ctx))]
		ctx.PushBoolean(ok)
		return 1
	})
	...
	tracer.vm.PushGoFunction(func(ctx *duktape.Context) int { ctx.PushUint(*tracer.resourceValue); return 1 })
	tracer.vm.PutPropString(logObject, "getResource")
...
func (jst *Tracer) CaptureStart(from common.Address, to common.Address, create bool, input []byte, resource uint64, value *big.Int) error {
	jst.ctx["type"] = "CALL"
	if create {
		jst.ctx["type"] = "CREATE"
	}
...
func (jst *Tracer) CaptureState(env *vm.VM, pc uint64, op vm.OpCode, resource, cost uint64, memory *vm.Memory, stack *vm.Stack, contract *vm.Contract, depth int, err error) error {
	if jst.err == nil {
		// Initialize the context if it wasn't done yet
		if !jst.initialized {
			jst.ctx["block"] = env.BlockNumber.Uint64()
			jst.initialized = true
		}
		...
		*jst.pcValue = uint(pc)
		*jst.resourceValue = uint(resource)
...
func (jst *Tracer) CaptureFault(env *vm.VM, pc uint64, op vm.OpCode, resource, cost uint64, memory *vm.Memory, stack *vm.Stack, contract *vm.Contract, depth int, err error) error {
	if jst.err == nil {
