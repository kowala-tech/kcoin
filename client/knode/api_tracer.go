...
// TraceChain returns the structured logs created during the execution of VM
...
func (api *PrivateDebugAPI) TraceChain(ctx context.Context, start, end rpc.BlockNumber, config *TraceConfig) (*rpc.Subscription, error) {
	...
	default:
		to = api.kcoin.blockchain.GetBlockByNumber(uint64(end))
	}
...
func (api *PrivateDebugAPI) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	database := state.NewDatabase(api.kcoin.ChainDb())

	if number := start.NumberU64(); number > 0 {
		start = api.kcoin.blockchain.GetBlock(start.ParentHash(), start.NumberU64()-1)
		if start == nil {
		...
		go func() {
			...
			for task := range tasks {
				...
				for i, tx := range task.block.Transactions() {
					...
					vmctx := core.NewVMContext(msg, task.block.Header(), api.kcoin.blockchain, nil)
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			block := api.kcoin.blockchain.GetBlockByNumber(number)
			if block == nil {
			...
			_, _, _, err := api.kcoin.blockchain.Processor().Process(block, statedb, vm.Config{})
			if err != nil {
				failed = err
				break
			}
...
// VM and returns them as a JSON object.
func (api *PrivateDebugAPI) TraceBlockByNumber(ctx context.Context, number rpc.BlockNumber, config *TraceConfig) ([]*txTraceResult, error) {
	...
	default:
		block = api.kcoin.blockchain.GetBlockByNumber(uint64(number))
	}
...
// TraceBlock returns the structured logs created during the execution of VM
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	if err := api.kcoin.engine.VerifyHeader(api.kcoin.blockchain, block.Header(), true); err != nil {
		return nil, err
	}
	...
	for i, tx := range txs {
		...
		if _, _, _, err := core.ApplyMessage(vmenv, msg, new(core.CompResourcesPool).AddResources(msg.ComputationalEffort())); err != nil {
			failed = err
			break
		}
		// Finalize the state so any modifications are written to the trie
...
func (api *PrivateDebugAPI) computeStateDB(block *types.Block, reexec uint64) (*state.StateDB, error) {
	...
	for i := uint64(0); i < reexec; i++ {
		block = api.kcoin.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
		if block == nil {
			break
		}
	...
	for block.NumberU64() < origin {
		...
		if block = api.kcoin.blockchain.GetBlockByNumber(block.NumberU64() + 1); block == nil {
...
// TraceTransaction returns the structured logs created during the execution of VM
// and returns them as a JSON object.
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	// Retrieve the transaction and assemble its VM context
	tx, blockHash, _, index := rawdb.ReadTransaction(api.kcoin.ChainDb(), hash)
	if tx == nil {
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash common.Hash, txIndex int, reexec uint64) (core.Message, vm.Context, *state.StateDB, error) {
	...
	for idx, tx := range block.Transactions() {
		...
		msg, _ := tx.AsMessage(signer)
		context := core.NewVMContext(msg, block.Header(), api.kcoin.blockchain, nil)
		if idx == txIndex {
			return msg, context, statedb, nil
		}
		// Not yet the searched for transaction, execute on top of the current state
		vmenv := vm.New(context, statedb, api.config, vm.Config{})
		if _, _, _, err := core.ApplyMessage(vmenv, msg, new(core.CompResourcesPool).AddResources(tx.ComputeLimit())); err != nil {
